...
# --- Charger les séries ---
hycom = read.table("C:/Users/jdanielou/Desktop/plot_internship/ts_csv/hycom/hycom_zone_3.4.csv")$V1
ersst  = read.table("C:/Users/jdanielou/Desktop/plots_internship/ts_csv/ersst/ersst_zone_3.4.csv")$V1
ersst_sub = ersst[1681:1944]
dates_hycom = seq(from = as.Date("1994-01-01"), by = "month", length.out = length(hycom))
dates_ersst  = seq(from = as.Date("1854-01-01"), by = "month", length.out = length(ersst))

# --- Calcul moyenne glissante 3 mois pour HYCOM ---
hycom_3mois = rollmean(hycom, k = 3, align = "center")
ersst_3mois = rollmean(ersst_sub, k = 3, align = "center")
dates_3mois = dates_hycom[2:(length(dates_hycom) - 1)]
...
oni_hycom = numeric(length(hycom_3mois))

for (i in seq_along(dates_3mois)) {
  date_i = dates_3mois[i]
  mois_i = as.numeric(format(date_i, "%m"))
  clim = get_climatology(date_i)
  oni_hycom[i] = hycom_3mois[i] - ((clim[ifelse(mois_i-1==0,12,mois_i-1)] + clim[mois_i] + clim[ifelse(mois_i + 1 == 13, 1, mois_i+1)])/3)
}
...
#------------ PLOT COMPARAISON HYCOM vs ERSST --------------------------
...
# Initialiser le plot
plot(dates_3mois, oni_hycom, type = "n", ylim = c(-2.79, 2.79), xlim = c(9077, 16447),
     xlab = "", ylab = "",
     xaxt = "n", yaxt = "n", bty = "n",
     main = paste("Comparison of ONI indexes - HYCOM vs ERSST", "( RMSE = ",round(sqrt(mean((oni_hycom - oni_ersst)^2, na.rm = TRUE)),3),")"),
...
text(10300, c(0.75, 1.25, 1.75, 2.25),
...
lines(dates_3mois, oni_hycom, col = "gold", lwd = 3)
...
# Légende
legend(x = 14800, y = 2.6, legend = c("ONI HYCOM", "ONI ERSST"), text.col = "gray99",
       col = c("gold", "purple"),
       lty = c(1, 2), lwd = 2.5,
       bty = "0", bg= "gray55" ,cex = 1.3)

dev.copy(png,file="C:/Users/jdanielou/Desktop/plots_internship/plot/oni_index/hycom_oni_index.png", width = 15, height = 9, units = "in", res = 150 )
dev.off()
