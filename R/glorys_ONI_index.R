...
# --- Charger les séries ---
glorys = read.table("C:/Users/jdanielou/Desktop/plot_internship/ts_csv/glorys/glorys_zone_3.4.csv")$V1
ersst  = read.table("C:/Users/jdanielou/Desktop/plots_internship/ts_csv/ersst/ersst_zone_3.4.csv")$V1
ersst_sub = ersst[1669:2016]
dates_glorys = seq(from = as.Date("1993-01-01"), by = "month", length.out = length(glorys))
dates_ersst  = seq(from = as.Date("1854-01-01"), by = "month", length.out = length(ersst))

# --- Calcul moyenne glissante 3 mois pour GLORYS ---
glorys_3mois = rollmean(glorys, k = 3, align = "center")
ersst_3mois = rollmean(ersst_sub, k = 3, align = "center")
dates_3mois = dates_glorys[2:(length(dates_glorys) - 1)]
...
oni_glorys = numeric(length(glorys_3mois))

for (i in seq_along(dates_3mois)) {
  date_i = dates_3mois[i]
  mois_i = as.numeric(format(date_i, "%m"))
  clim = get_climatology(date_i)
  oni_glorys[i] = glorys_3mois[i] - ((clim[ifelse(mois_i-1==0,12,mois_i-1)] + clim[mois_i] + clim[ifelse(mois_i + 1 == 13, 1, mois_i+1)])/3)
}
...
#------------ PLOT COMPARAISON GLORYS vs ERSST --------------------------
...
# Initialiser le plot
plot(dates_3mois, oni_glorys, type = "n", ylim = c(-2.79, 2.79), xlim = c(8811, 18557),
     xlab = "", ylab = "",
     xaxt = "n", yaxt = "n", bty = "n",
     main = paste("Comparison of ONI indexes - GLORYS vs ERSST", "( RMSE = ",round(sqrt(mean((oni_glorys - oni_ersst)^2, na.rm = TRUE)),3),")"),
...
lines(dates_3mois, oni_glorys, col = "gold", lwd = 3)
...
# Légende
legend("topright", legend = c("ONI GLORYS", "ONI ERSST"), text.col = "gray99",
       col = c("gold", "purple"),
       lty = c(1, 2), lwd = 2.5,
       bty = "0", bg= "gray55" ,cex = 1.3)

dev.copy(png,file="C:/Users/jdanielou/Desktop/plots_internship/plot/oni_index/glorys_oni_index.png", width = 15, height = 9, units = "in", res = 150 )
dev.off()
